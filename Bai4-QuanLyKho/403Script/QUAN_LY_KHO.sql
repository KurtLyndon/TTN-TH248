...
Create proc INSERTHANGHOA(@ID CHAR(10),@NAME NVARCHAR(50),@GIA BIGINT,@IDNSX NVARCHAR(50),@CHECK INT OUTPUT)
 AS
  BEGIN
   IF(@ID IN (SELECT HANGHOA.ID_HangHoa FROM HANGHOA)) BEGIN SET @CHECK=1 RETURN END
   IF(@IDNSX NOT IN (SELECT NHACUNGCAP.ID_NhaCC FROM NHACUNGCAP))BEGIN SET @CHECK=2 RETURN END
    SET @CHECK=0
	  INSERT INTO HANGHOA VALUES(@ID,@NAME,@GIA,@IDNSX) 
  END
GO
CREATE PROC DELETEHANGHOA(@ID CHAR(10))
 AS
  BEGIN
  DELETE HANGHOA
  WHERE @ID=HANGHOA.ID_HangHoa 
 END
GO
CREATE PROC DELETENHACUNGCAP (@ID CHAR(10))
 AS
  BEGIN
  DELETE NHACUNGCAP
  WHERE @ID=NHACUNGCAP.ID_NhaCC 
 END
GO
CREATE PROC DELETEPHIEUNHAP (@ID CHAR(10))
 AS
  BEGIN 
 ...
 END
GO
CREATE PROC DELETECHITIETPHIEU (@ID CHAR(10))
 AS
  BEGIN 
  DELETE CHITIETPHIEU WHERE @ID=CHITIETPHIEU.ID_Phieu
 END
GO
CREATE PROC INSERTNHACUNGCAP(@ID CHAR(10),@NAME NVARCHAR(50),@SDT VARCHAR(20),@EMAIL NVARCHAR(50),@ADDRESS NVARCHAR(50),@CHECK INT OUTPUT)
 AS
  BEGIN
  IF(@ID IN (SELECT ID_NhaCC FROM NHACUNGCAP))BEGIN SET @CHECK=1 RETURN END
  SET @CHECK=0
   INSERT INTO NHACUNGCAP VALUES(@ID,@NAME,@SDT,@EMAIL,@ADDRESS)
 END
GO
   CREATE PROC INSERTCHITIETPHIEU (@ID CHAR(10),@IDHH CHAR(10),@GC NVARCHAR(50),@CHECK INT OUTPUT)
   AS
   BEGIN
    IF(@ID NOT IN (SELECT ID_Phieu FROM CHITIETPHIEU))BEGIN SET @CHECK=1 RETURN END
	IF(@IDHH NOT IN (SELECT ID_HangHoa FROM HANGHOA))BEGIN SET @CHECK=2 RETURN END
	SET @CHECK=0
	INSERT INTO CHITIETPHIEU VALUES(@ID,@IDHH,@GC)
	END
	GO
	CREATE PROC INSERTPHIEUNHAP(@ID CHAR(10),@DATE DATETIME,@TYPE VARCHAR(50),@CHECK INT OUTPUT)
	AS
	BEGIN
	 IF(@ID IN (SELECT ID_Phieu FROM PHIEU))BEGIN SET @CHECK=1 RETURN END
	 SET @CHECK=0
	 INSERT INTO PHIEU VALUES (@ID,@DATE,@TYPE)
	 CREATE PROC UPDATEPHIEU(@ID CHAR(10),@DATE DATETIME,@TYPE VARCHAR(50),@CHECK INT OUTPUT)
	 AS
	 BEGIN 
	  SET @CHECK=0
	   UPDATE PHIEU SET
		...
		END
		GO
	CREATE PROC UPDATENHACUNGCAP(@ID CHAR(10),@NAME NVARCHAR(50),@SDT VARCHAR(20),@EMAIL NVARCHAR(50),@ADDRESS NVARCHAR(50),@CHECK INT OUTPUT)
	AS
	 BEGIN 
	  SET @CHECK=0
	   ...
	   END
GO
Create proc UPDATEHANGHOA(@ID CHAR(10),@NAME NVARCHAR(50),@GIA BIGINT,@IDNSX NVARCHAR(50),@CHECK INT OUTPUT)
 AS
  BEGIN
   IF(@IDNSX NOT IN (SELECT NHACUNGCAP.ID_NhaCC FROM NHACUNGCAP))BEGIN SET @CHECK=2 RETURN END
   SET @CHECK=0
   UPDATE HANGHOA SET
    ...
    END
GO
CREATE PROC UPDATECHITIETPHIEU (@ID CHAR(10),@IDHH CHAR(10),@GC NVARCHAR(50),@CHECK INT OUTPUT)
  AS
   BEGIN
    IF(@ID NOT IN (SELECT ID_Phieu FROM CHITIETPHIEU))BEGIN SET @CHECK=1 RETURN END
	IF(@IDHH NOT IN (SELECT ID_HangHoa FROM HANGHOA))BEGIN SET @CHECK=2 RETURN END
	SET @CHECK=0
	UPDATE CHITIETPHIEU SET 
	ID_HangHoa=@IDHH,
	ID_Phieu=@ID,
	GIA=@GC
	WHERE ID_HangHoa=@IDHH AND ID_Phieu=@ID
   END
GO