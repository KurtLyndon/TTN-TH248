...
CREATE PROC INSERTHANGHOA(@ID CHAR(10),@NAME NVARCHAR(50),@COUT INT ,@COST BIGINT,@CHECK INT OUTPUT)
AS
  BEGIN
  ...
  END
  CREATE PROC UPDATEHANGHOA(@ID CHAR(10),@NAME NVARCHAR(50),@COUT INT ,@COST BIGINT,@CHECK INT OUTPUT)
  AS
    BEGIN
	SET @CHECK=0
	 UPDATE HANGHOA 
	 SET HANGHOA.SoLuong=@COUT,
	  HANGHOA.Gia=@COST,
	  HANGHOA.TenHangHoa=@NAME
...
CREATE PROC DELETEHANGHOA(@ID CHAR(10))
 AS
   BEGIN 
	...
	END
	CREATE PROC INSERTKHACHHANG(@ID CHAR(10),@NAME NVARCHAR(50),@CMTND NVARCHAR(20),@SDT CHAR(20),@CHECK INT OUTPUT)
	AS
	BEGIN 
	  IF(@ID IN (SELECT KHACHHANG.ID_KhachHang FROM KHACHHANG)) BEGIN SET @CHECK=1 RETURN END
	  SET @CHECK=0
	  INSERT INTO KHACHHANG VALUES (@ID,@NAME,@CMTND,@SDT)
	  END
	  CREATE PROC UPDATEKHACHHANG(@ID CHAR(10),@NAME NVARCHAR(50),@CMTND NVARCHAR(20),@SDT CHAR(20),@CHECK INT OUTPUT)
	 AS
	 BEGIN 
	  SET @CHECK=0
	   UPDATE KHACHHANG SET TenKhachHang=@NAME,CMND=@CMTND,SDT=@SDT
	   WHERE @ID=ID_KhachHang
	   END
	   CREATE PROC DELETEKHACHHANG(@ID CHAR(10))
	   AS
	    BEGIN 
		 DELETE KHACHHANG WHERE ID_KhachHang=@ID
		 DELETE HOADON WHERE ID_KhachHang=@ID
		 END
		 CREATE PROC INSERTNHANVIEN(@ID CHAR(10),@NAME NVARCHAR(50),@CMTND NVARCHAR(20),@SDT CHAR(20),@CHECK INT OUTPUT)
	AS
	BEGIN 
	  IF(@ID IN (SELECT NHANVIEN.ID_NHANVIEN FROM NHANVIEN)) BEGIN SET @CHECK=1 RETURN END
	  SET @CHECK=0
	  INSERT INTO NHANVIEN VALUES (@ID,@NAME,@CMTND,@SDT)
	  END
	  CREATE PROC UPDATENHANVIEN(@ID CHAR(10),@NAME NVARCHAR(50),@CMTND NVARCHAR(20),@SDT CHAR(20),@CHECK INT OUTPUT)
	 AS
	 BEGIN 
	  SET @CHECK=0
	   UPDATE NHANVIEN SET TenNHANVIEN=@NAME,CMND=@CMTND,SDT=@SDT
	   WHERE @ID=ID_NHANVIEN
	   END
	   CREATE PROC DELETENHANVIEN(@ID CHAR(10))
	   AS
	    BEGIN 
		 DELETE NHANVIEN WHERE ID_NHANVIEN=@ID
		 DELETE HOADON WHERE ID_NHANVIEN=@ID
		 END
		 CREATE PROC INSERTHOADON(@ID CHAR(10),@ID_KH CHAR(10),@ID_NV CHAR(10),@DATE DATETIME,@CHECK INT OUTPUT)
		 AS
		 BEGIN
		 IF(@ID IN (SELECT A.ID_HoaDon FROM HOADON AS A))BEGIN SET @CHECK=1 RETURN END
		 IF(@ID_KH NOT IN (SELECT A.ID_KhachHang FROM KHACHHANG AS A) )BEGIN SET @CHECK=2 RETURN END
		  IF(@ID_NV NOT IN (SELECT A.ID_NHANVIEN FROM NHANVIEN AS A) )BEGIN SET @CHECK=3 RETURN END
		  IF(@ID_KH IN (SELECT A.ID_HoaDon FROM HOADON AS A ) AND @ID_NV IN (SELECT B.ID_NhanVien FROM HOADON AS B))BEGIN SET @CHECK=4 RETURN END
		  SET @CHECK=0
		  INSERT INTO HOADON VALUES (@ID,@ID_NV,@ID_KH,@DATE)
		 END
		 CREATE PROC UPDATEHOADON(@ID CHAR(10),@ID_KH CHAR(10),@ID_NV CHAR(10),@DATE DATETIME,@CHECK INT OUTPUT)
		 AS
		  BEGIN
		   IF(@ID_KH NOT IN (SELECT A.ID_KhachHang FROM KHACHHANG AS A) )BEGIN SET @CHECK=2 RETURN END
		  IF(@ID_NV NOT IN (SELECT A.ID_NHANVIEN FROM NHANVIEN AS A) )BEGIN SET @CHECK=3 RETURN END
		  IF(@ID_KH IN (SELECT A.ID_HoaDon FROM HOADON AS A WHERE A.ID_HoaDon <> @ID) AND @ID_NV IN (SELECT B.ID_NhanVien FROM HOADON AS B WHERE B.ID_HoaDon <> @ID))BEGIN SET @CHECK=4 RETURN END
		  SET @CHECK=0
		    UPDATE HOADON SET ID_KhachHang=@ID_KH,ID_NhanVien=@ID_NV,Ngay=@DATE
			WHERE ID_HoaDon=@ID
		  END 
		   CREATE PROC DELETEHOADON(@ID CHAR(10))
		   AS
		    BEGIN
			 DELETE HOADON WHERE ID_HoaDon=@ID
			 DELETE CHITIETHOADON WHERE ID_HoaDon=@ID
			 END
			 CREATE PROC INSERTCHITIETHOADON(@ID CHAR(10),@ID_HH CHAR(10),@COUT INT,@CHECK INT OUTPUT)
			 AS
			  BEGIN
			    IF(@ID IN (SELECT A.ID_HoaDon FROM CHITIETHOADON A)AND @ID_HH IN (SELECT B.ID_HANGHOA FROM CHITIETHOADON B))BEGIN SET @CHECK=1 RETURN END
			    IF(@ID_HH NOT IN (SELECT A.ID_HangHoa FROM HANGHOA AS A) )BEGIN SET @CHECK=2 RETURN END
				SET @CHECK=0
				INSERT INTO CHITIETHOADON VALUES(@ID,@ID_HH,@COUT)
			  END
			  CREATE PROC UPDATECHITIETHOADON(@ID CHAR(10),@ID_HH CHAR(10),@COUT INT,@CHECK INT OUTPUT)
			    AS
				BEGIN
				SET @CHECK=0
				 UPDATE CHITIETHOADON SET SoLuong=@COUT WHERE ID_HangHoa=@ID_HH AND ID_HoaDon=@ID
				 END
				 CREATE PROC DELETECHITIETHOADON(@ID CHAR(10),@ID_HH CHAR(10))
				   AS
				    BEGIN 
					DELETE CHITIETHOADON WHERE ID_HoaDon=@ID AND ID_HangHoa=@ID_HH
					END
					CREATE PROC LOGINUSER(@USER NVARCHAR(50),@PASS NVARCHAR(50),@CHECK INT OUTPUT)
					AS
					 BEGIN
					   IF(@USER IN (SELECT A.Username FROM DANGNHAP A) AND @PASS IN (SELECT B.Pass FROM DANGNHAP B))BEGIN SET @CHECK=0 RETURN END
					   ELSE SET @CHECK=1 
					   END